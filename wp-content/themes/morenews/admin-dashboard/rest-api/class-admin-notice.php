    ...
    function morenews_dashboard_notice_content(){
		if ( file_exists( WP_PLUGIN_DIR . '/af-companion/af-companion.php' ) ) {
			if(is_plugin_active('af-companion/af-companion.php')){
				$af_companion_title = __( 'Get Starter Sites', 'morenews' );
				$af_companion_url = site_url( ).'/wp-admin/admin.php?page=af-companion';
			}else{
            $af_companion_title = __( 'Activate AF Companion', 'morenews' );
            ...
            $af_companion_title = __( 'Install AF Companion', 'morenews' );
		...
		$demo_link = "https://afthemes.com/products/morenews/";
		$notice_starter_msg =sprintf(
			...
			__( 'General info', 'morenews' ),
			esc_html__( 'All of the features provided by the theme are now ready to use; Here, we have gathered all of the essential details and helpful links for you and your better experience with the theme. Once again, Thanks for using our theme!', 'morenews' ),
			$af_companion_url,
			$af_companion_title,
			esc_url($demo_link),
		...
		$notice_external_msg =sprintf(
			...
			esc_html__( 'Return to your dashboard', 'morenews' )
